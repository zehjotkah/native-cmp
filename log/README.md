...
1. **Deploy**: click the button, sign in to Cloudflare and confirm. The Worker and its database are created for you, and you set the export token (`EXPORT_TOKEN`) during setup. Or deploy from a clone with `npx wrangler@4 deploy`.
2. **Use your own domain**: in the Cloudflare dashboard, add a custom domain such as `consentlog.yourdomain.com` to the Worker (Settings → Domains & Routes). Otherwise the log is a third-party domain for your visitors, which you would have to declare separately.
3. **Point your site at it**: in Webstudio, Project Settings → Custom Code:
   ...
   ```
4. **Restrict writers** (recommended): set `ALLOWED_ORIGINS` to your site, e.g. `https://example.com,https://www.example.com`. Without it, entries are accepted from any site and the origin is stored with each entry.
...
| Name | Default | |
| --- | --- | --- |
| `RETENTION_DAYS` | `1095` | How long entries are kept |
| `ALLOWED_ORIGINS` | empty | Comma-separated sites allowed to write; empty accepts any |
