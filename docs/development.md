...
| Proof of consent | Each decision stores a random consent ID in `cmp_consent`; the dialog shows it (`data-cmp-consent-id`, tokens `consent-id`, `consent-id-value`, text `modal.consentId`). `cmpConfig.consentLog` posts one entry per decision (consent ID, time, type, choices, config fingerprint, language) with `sendBeacon`; never an IP address. The optional Worker lives in its own repository, [native-cmp-consent-log](https://github.com/zehjotkah/native-cmp-consent-log) (D1, `RETENTION_DAYS`, `ALLOWED_ORIGINS`, `EXPORT_TOKEN`, `npm test`); Cloudflare's deploy flow rejects subdirectory URLs, so it cannot live in this repository. ELECOS's own deployment runs at `consentlog.nativecmp.com`. Covered on `/generator` (step 7), `/examples` (Consent log), `/docs` (Proof of consent), Home FAQ, `install.txt` (4c) and `install.mjs` (`--consent-log`). Engine writes the ID only after the first interaction or when the dialog opens, so hydration is not disturbed. |
