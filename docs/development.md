...
| Proof of consent | Each decision stores a random consent ID in `cmp_consent`; the dialog shows it (`data-cmp-consent-id`, tokens `consent-id`, `consent-id-value`, text `modal.consentId`). `cmpConfig.consentLog` posts one entry per decision (consent ID, time, type, choices, config fingerprint, language) with `sendBeacon`; never an IP address. The optional Worker lives in `log/` (D1, `RETENTION_DAYS`, `ALLOWED_ORIGINS`, `EXPORT_TOKEN`, `cd log && npm test`, `npm run setup` for a one-command install); the deploy button uses the subdirectory URL `.../tree/main/log`. ELECOS's own deployment runs at `consentlog.nativecmp.com`, configured in `log/wrangler.nativecmp.jsonc` (untracked). Note: browser extensions that strip `ref=` query parameters break Cloudflare's deploy flow, because it reads the folder via `contents/log?ref=main`. Covered on `/generator` (step 7), `/examples` (Consent log), `/docs` (Proof of consent), Home FAQ, `install.txt` (4c) and `install.mjs` (`--consent-log`). Engine writes the ID only after the first interaction or when the dialog opens, so hydration is not disturbed. |
| Communication order | AI install → generator → examples → documentation (→ starter project as the fifth way on Home and `/install`), on Home (hero, Install section, FAQ), in the navigation, footer, docs (Installation section), generator and examples intros, and `llms.txt`. |
...
| Asset `native-cmp-logo.svg` | Brand logo in the Site Header (Image component, token `site-logo`: 48 px, 40 px on phones). Cropped copy of `assets/source/Native_CMP_Logo.svg`, kept as `assets/native-cmp-logo.svg`. |
