...
- **Consent log Worker** ([native-cmp-consent-log](https://github.com/zehjotkah/native-cmp-consent-log)): a Cloudflare Worker with a D1 database, deployed with one click into your own account. Automatic deletion after `RETENTION_DAYS` (3 years), an origin allowlist, a CSV export protected by a token, and its own end-to-end tests.
